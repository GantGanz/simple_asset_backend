package com.test.simpleasset.config;
...
import com.test.simpleasset.filter.AuthorizationFilter;
...
//		Uncomment the 'users/**' matchers to create super admin for the first time. Then comment back when it's been created
//		matchers.add(new AntPathRequestMatcher("/users/**", HttpMethod.POST.name()));
	...
	@Bean
	public SecurityFilterChain securityFilterChain(final HttpSecurity http, final AuthorizationFilter authorizationFilter) throws Exception {
		http.cors();
		http.csrf().disable();
